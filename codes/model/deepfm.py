...
class DeepFM(nn.Module, EmbeddingMixin):
    ...
    def __init__(self, features, num_classes, embedding_size, hidden_layers,
                 ...
                 use_linear=True, linear_hidden = [256],use_fm=True, use_deep=True):
        ...
        if self.use_linear and self.features.number_features:
            self.linear = MLP(
                1024,
                hidden_layers=linear_hidden, dropout=0.1, activation='relu')
            final_layer_input_size += linear_hidden[-1]
        ...
        if use_deep and total_embedding_sizes:
            total_input_size = (total_embedding_sizes + 1024)
            self.mlp = MLP(
    ...
    def forward(self, x):
        ...
        if self.linear:
            # linear
            for feature in self.features.number_features:
                number_inputs.append(x[feature.name])
        ...
        elif self.final_activation == 'direct_output':
            pass
